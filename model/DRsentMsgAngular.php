...
}  else
{
	...
	$from=$_SESSION['DID'];
    $PID = mysqli_real_escape_string($connection_link,$data->to_id);
    $subject = mysqli_real_escape_string($connection_link,$data->subject);
    $msg = mysqli_real_escape_string($connection_link,$data->msg);
	
	$sql="INSERT INTO `sent_messages`( `PID`, `DID`, `SendDATE`, `SUBJECT`, `CONTENT`, `Sender`)VALUES ('".$PID."','".$from."', '".$Date."', '".$subject."', '".$msg."','".$from."')";
